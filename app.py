...
# VERIFICAÇÃO DE LOGIN AUTOMÁTICO (COOKIE) NO TOPO
if cookies_enabled and not st.session_state.logged_in and "code" not in st.query_params:
...
# ==========================================
# MÓDULO 1: TELA DE LOGIN (ISOLADA)
# ==========================================
if not st.session_state.logged_in:
    
    # 🚨 INTERCEPTADOR GOOGLE BLINDADO 🚨
    if "code" in st.query_params:
        codigo_autorizacao = st.query_params.get("code")
        ph = st.empty()
        ph.info("🔄 Validando acesso com o Google...")
            ...
            if res.status_code == 200:
                ...
                if user_res.status_code == 200:
                    ...
                    if cookies_enabled: 
                        ...
                        time.sleep(0.5) # Dá tempo ao navegador gravar o cookie
                    
                    st.query_params.clear() # Só limpa se deu 100% certo!
                    ph.empty()
                    st.rerun()
                else:
                    ph.error("❌ Falha ao ler seu perfil do Google.")
                    time.sleep(2)
                    st.query_params.clear()
                    st.rerun()
            else:
                ph.error("❌ Código do Google expirado ou inválido. Tente novamente.")
                time.sleep(2)
                st.query_params.clear()
                st.rerun()
        except Exception as e: 
            ph.error("❌ Erro nos servidores do Google. Tente novamente.")
            time.sleep(2)
            st.query_params.clear()
            st.rerun()

    # Só exibe o formulário de login se o interceptador não estiver rodando
